...
function UrgencyBadge({ urgency }) {
  ...
  return <span className={`badge ${map[urgency] || 'badge-low'}`}>● {urgency}</span>;
}
...
function CategoryChip({ category }) {
  ...
  return <span className="chip">{labels[category] || category}</span>;
}
...
function EditModal({ email, onClose, onSend }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-header">✏️ Edit reply to {email.sender_name}</div>
...
export default function EmailCard({ email, onAction }) {
  const [expanded, setExpanded] = useState(email.needs_reply);
  const [editOpen, setEditOpen] = useState(false);
  const [viewOpen, setViewOpen] = useState(false);
  const [status, setStatus] = useState(email.draft?.status || null);
  ...
  const handleSend = (emailId, body) => {
    ...
    setTimeout(() => {
      setSending(false);
      setStatus('sent');
      onAction && onAction(emailId, 'sent');
    }, 900);
  };
  ...
  return (
    <>
      <div
        ...
        style={{
          marginBottom: 16,
          borderLeft: email.needs_reply ? '3px solid var(--accent)' : '3px solid transparent',
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          <SenderAvatar name={email.sender_name} email={email.sender_email} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
              <span style={{ fontWeight: 700, fontSize: 14 }}>{email.sender_name}</span>
              <span style={{ fontSize: 12, color: 'var(--muted)', flexShrink: 0 }}>{timeAgo(email.received_at)}</span>
            </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', margin: '12px 0 0' }}>
          ...
          {email.needs_reply && <span className="badge badge-purple">💬 Needs reply</span>}
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 14, lineHeight: 1.65, color: 'var(--text)' }}>{email.ai_summary}</p>
          </div>
        </div>
              ...
              <div className="fade-in">
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
                  <button
                    ...
                    onClick={() => handleSend(email.id, email.draft.body)}
                    disabled={sending}
                  >
