...
const ACCOUNTS_PATH = path.join(process.cwd(), '.mailmind-accounts.json');
...
/**
 * Normalize and clean passwords: automatically handles 16-character spaced or dashed App Passwords (e.g. from Google or Apple)
 */
function sanitizePassword(password, email, provider) {
  if (password == null) return '';
  const raw = typeof password === 'string' ? password : String(password);
 ...
 * When explicit credentials are provided, avoids cross-account fallback.
 */
function resolveClientCredentials(credentials) {
  ...
  const hasExplicitEmail = creds.email != null || creds.user != null || creds.username != null;
  const hasExplicitPass = creds.password != null || creds.pass != null || creds.appPassword != null;

  let emailUser = '';
  if (hasExplicitEmail) {
    emailUser = String(creds.email || creds.user || creds.username || '').trim();
  } else {
    emailUser = String(localConfig.email || process.env.EMAIL_USER || process.env.GMAIL_USER || process.env.MAILMIND_EMAIL || '').trim();
  }

  // Look up user in saved accounts if password is not explicitly provided or to complete configuration
  let matchedAccount = null;
  if (emailUser) {
    if (localConfig.email && localConfig.email.toLowerCase() === emailUser.toLowerCase()) {
      matchedAccount = localConfig;
    } else {
      matchedAccount = findSavedAccount(emailUser);
    }
  }

  let rawPass = '';
  if (hasExplicitPass) {
    rawPass = creds.password != null ? String(creds.password) : (creds.pass != null ? String(creds.pass) : String(creds.appPassword || ''));
  } else if (matchedAccount && (matchedAccount.password || matchedAccount.pass)) {
    rawPass = String(matchedAccount.password || matchedAccount.pass || '');
  } else if (!hasExplicitEmail) {
    // Only fall back to local config or env if NO explicit email was passed
    rawPass = String(localConfig.password || process.env.EMAIL_PASS || process.env.GMAIL_APP_PASSWORD || process.env.MAILMIND_PASSWORD || '');
  }

  const provider = (creds.provider || matchedAccount?.provider || localConfig.provider || process.env.EMAIL_PROVIDER || 'gmail').toLowerCase();
  const emailPass = sanitizePassword(rawPass, emailUser, provider);

  if (!emailUser || !emailPass) {
    console.error('ERROR: Missing email address or password.');
  }

  const host = creds.host || matchedAccount?.host || localConfig.host || process.env.EMAIL_HOST || null;
  const port = creds.port || matchedAccount?.port || localConfig.port || process.env.EMAIL_PORT || 993;
  const tone = creds.tone || matchedAccount?.tone || localConfig.tone || 'professional';
  const monitoringMode = creds.monitoringMode || matchedAccount?.monitoringMode || localConfig.monitoringMode || process.env.MAILMIND_MONITORING_MODE || 'ask_permission';
...
async function getMailClient(credentials = {}) {
  ...
  if (!isValidEmail(emailUser)) {
    throw new Error(`Invalid email address format: "${emailUser}". Please enter a valid email address.`);
  }
  if (!emailPass) {
    throw new Error('Password is required. Please enter the exact password of your email account to sign in.');
  }
...
async function testConnection(credentials) {
  ...
  let creds;
  try {
    creds = resolveClientCredentials(credentials);
    if (!creds.emailUser) {
      return { success: false, error: 'Email address is required.' };
    }
    if (!isValidEmail(creds.emailUser)) {
      return { success: false, error: 'Invalid email address format. Please enter a valid email address (e.g. yourname@domain.com).' };
    }
    if (!creds.emailPass) {
      return { success: false, error: 'Password is required. Please enter the exact password of your email account to sign in.' };
    }
    ...
    let userFriendlyError = err.message || 'Failed to authenticate with email server.';
    const lower = userFriendlyError.toLowerCase();
    const effectiveCreds = creds || resolveClientCredentials(credentials);
    const isGoogle = effectiveCreds && (effectiveCreds.provider === 'google' || effectiveCreds.provider === 'gmail' || (effectiveCreds.emailUser && effectiveCreds.emailUser.toLowerCase().includes('gmail')));
    if (lower.includes('invalid credentials') || lower.includes('authenticationfailed') || lower.includes('username and password not accepted') || lower.includes('auth error') || lower.includes('login failed') || lower.includes('failure')) {
      if (isGoogle) {
        userFriendlyError = 'Authentication failed: Google requires a 16-character App Password (not your standard Google account password). Please generate a 16-character App Password manually at https://myaccount.google.com/apppasswords and paste it here.';
      } else {
        userFriendlyError = 'Authentication failed: The email address or password entered is incorrect. Please verify your credentials or generate a 16-character App Password from your provider.';
      }
    } else if (lower.includes('enotfound') || lower.includes('getaddrinfo') || lower.includes('could not reach')) {
      userFriendlyError = 'Unable to reach the email server. Please check the email domain or your network connection.';
    }
...
function loadLocalConfig() {
  ...
  if ((process.env.EMAIL_USER || process.env.GMAIL_USER || process.env.MAILMIND_EMAIL) &&
    ...
    const rawEnvPass = process.env.EMAIL_PASS || process.env.GMAIL_APP_PASSWORD || process.env.MAILMIND_PASSWORD || '';
    const envUser = process.env.EMAIL_USER || process.env.GMAIL_USER || process.env.MAILMIND_EMAIL || '';
    const envProv = process.env.EMAIL_PROVIDER || process.env.MAILMIND_PROVIDER || 'gmail';
    return {
      email: envUser,
      password: sanitizePassword(rawEnvPass, envUser, envProv),
      provider: envProv,
...
function loadLocalAccounts() {
  ...
  // Ensure current active user from CONFIG_PATH is in the list
  try {
    const active = loadLocalConfig();
    if (active && active.email && active.password) {
      const cleanActive = active.email.toLowerCase().trim();
      const idx = accounts.findIndex(a => a && a.email && a.email.toLowerCase().trim() === cleanActive);
      if (idx !== -1) {
        accounts[idx] = { ...accounts[idx], ...active };
      } else {
...
function saveLocalAccounts(accounts) {
  try {
    const cleanAccounts = (accounts || []).filter(a => a && a.email);
    fs.writeFileSync(ACCOUNTS_PATH, JSON.stringify(cleanAccounts, null, 2), 'utf8');
    return true;
  } catch (err) {
    return false;
  }
...
function addOrUpdateSavedAccount(account) {
  ...
  const existingIdx = accounts.findIndex(a => a && a.email && a.email.trim().toLowerCase() === cleanEmail);
...
function findSavedAccount(email) {
  if (!email) return null;
  const clean = String(email).trim().toLowerCase();
  const accounts = loadLocalAccounts();
  const found = accounts.find(a => a && a.email && a.email.trim().toLowerCase() === clean);
  if (found) return found;

  const active = loadLocalConfig();
  if (active && active.email && active.email.trim().toLowerCase() === clean) {
    return active;
  }
  return null;
}

function removeSavedAccount(emailToRemove) {
  if (!emailToRemove) return false;
  const clean = String(emailToRemove).trim().toLowerCase();
  let accounts = loadLocalAccounts().filter(a => a && a.email && a.email.trim().toLowerCase() !== clean);
  saveLocalAccounts(accounts);
  const active = loadLocalConfig();
  if (active && active.email && active.email.trim().toLowerCase() === clean) {
    if (accounts.length > 0) {
      saveLocalConfig(accounts[0]);
    } else {
      clearLocalConfig();
    }
  }
...
function saveLocalConfig(config) {
  try {
    ...
    if (config && config.email) {
      addOrUpdateSavedAccount(config);
    }
    return true;
  } catch (err) {
    return false;
...
const PROVIDER_GUIDES = {
  google: {
    ...
    requires2FA: true,
    steps: [
      { step: 1, title: 'Ensure 2-Step Verification is ON', desc: 'Google requires 2-Step Verification before generating App Passwords.' },
      { step: 2, title: 'Open Google App Passwords page', desc: 'Visit https://myaccount.google.com/apppasswords in your browser.' },
      { step: 3, title: 'Name your App Password', desc: 'Enter "MailMind" as the App name and click Create.' },
      { step: 4, title: 'Copy the 16-character code', desc: 'Google will display a 16-character password (e.g. "abcd efgh ijkl mnop"). Copy it.' }
    ],
    tips: 'Google App Passwords are 16 letters displayed in 4 groups of 4.',
  ...
  microsoft: {
    ...
    requires2FA: true,
    steps: [
      { step: 1, title: 'Turn on Two-Step Verification', desc: 'Ensure Two-Step Verification is active on your Microsoft account security page.' },
      { step: 2, title: 'Go to Advanced Security Options', desc: 'Navigate to Additional Security Options -> App Passwords.' },
      { step: 3, title: 'Create new App Password', desc: 'Click "Create a new app password" under the App passwords section.' },
      { step: 4, title: 'Copy the password', desc: 'Copy the generated app password and use it here in MailMind.' }
    ],
  ...
  yahoo: {
    ...
    requires2FA: false,
    steps: [
      { step: 1, title: 'Open Yahoo Account Security', desc: 'Sign in to your Yahoo Account Security page.' },
      { step: 2, title: 'Generate App Password', desc: 'Scroll down and click "Generate app password".' },
      { step: 3, title: 'Label as MailMind', desc: 'Enter "MailMind" as your app name and click Generate.' },
      { step: 4, title: 'Copy and paste', desc: 'Copy the 16-character code into MailMind.' }
    ],
  ...
  icloud: {
    ...
    requires2FA: true,
    steps: [
      { step: 1, title: 'Sign in to Apple ID Management', desc: 'Go to https://appleid.apple.com and sign in with your Apple Account.' },
      { step: 2, title: 'Select App-Specific Passwords', desc: 'In the "Sign-In and Security" section, select "App-Specific Passwords".' },
      { step: 3, title: 'Generate Password', desc: 'Click "Generate an app-specific password" and enter "MailMind".' },
      { step: 4, title: 'Copy the password', desc: 'Copy the formatted code into MailMind.' }
    ],
  ...
  zoho: {
    ...
    requires2FA: true,
    steps: [
      { step: 1, title: 'Open Zoho Security Settings', desc: 'Log in to Zoho Accounts and go to Security -> Application-Specific Passwords.' },
      { step: 2, title: 'Generate New Password', desc: 'Click "Generate New Password", name it "MailMind", and click Generate.' },
      { step: 3, title: 'Copy the password', desc: 'Copy the generated password code and paste it here in MailMind.' }
    ],
    tips: 'Zoho accounts with TFA require an Application-Specific Password for IMAP access.',
  ...
  fastmail: {
    ...
    requires2FA: false,
    steps: [
      { step: 1, title: 'Open Fastmail App Passwords', desc: 'Go to Settings -> Privacy & Security -> App Passwords in Fastmail.' },
      { step: 2, title: 'New App Password', desc: 'Click "New App Password", choose "Mail (IMAP/SMTP)" access, and name it "MailMind".' },
      { step: 3, title: 'Copy token', desc: 'Copy the generated app password token into MailMind.' }
    ],
  ...
  custom: {
    ...
    requires2FA: false,
    steps: [
      { step: 1, title: 'Check your mail server settings', desc: 'Ensure IMAP (port 993 SSL) and SMTP (port 465 SSL or 587 STARTTLS) are enabled.' },
      { step: 2, title: 'Obtain or generate password / API token', desc: 'Use your mailbox password, app token, or generate a high-entropy random app key.' },
      { step: 3, title: 'Enter server details', desc: 'Provide your IMAP host (e.g. imap.yourdomain.com) and port in the connection form.' }
    ],
    tips: 'You can use the built-in Secure Password Generator to create high-entropy passwords for your custom mail accounts.',
    forgotPasswordTips: 'Generate a new high-entropy password using the generator, update it on your mail server or cPanel, and use it in MailMind.'
  }
};

function generateAppPassword(options = {}) {
...
function getProviderAppPasswordGuide(providerOrEmail) {
  if (!providerOrEmail) return PROVIDER_GUIDES.google;
  const val = String(providerOrEmail).toLowerCase().trim();
  if (val.includes('gmail') || val.includes('google')) return PROVIDER_GUIDES.google;
  if (val.includes('outlook') || val.includes('microsoft') || val.includes('office365') || val.includes('hotmail') || val.includes('live')) return PROVIDER_GUIDES.microsoft;
  if (val.includes('yahoo') || val.includes('ymail') || val.includes('rocketmail')) return PROVIDER_GUIDES.yahoo;
  if (val.includes('icloud') || val.includes('apple') || val.includes('me.com') || val.includes('mac.com')) return PROVIDER_GUIDES.icloud;
  if (val.includes('zoho')) return PROVIDER_GUIDES.zoho;
  if (val.includes('fastmail')) return PROVIDER_GUIDES.fastmail;
  if (val.includes('custom') || val.includes('imap') || val.includes('smtp')) return PROVIDER_GUIDES.custom;
  return PROVIDER_GUIDES.google;
}

function generateGoogleAppPassword() {
  return generateAppPassword({ format: 'spaced', length: 16 });
}
