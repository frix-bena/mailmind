...
function sanitizePassword(password, email, provider) {
  ...
  if (prov === 'gmail' || prov === 'google' || domain.includes('gmail') || domain.includes('googlemail') || !prov) {
    // If it contains spaces (like Gmail app password "xxxx xxxx xxxx xxxx"), strip all whitespace
    if (trimmed.includes(' ')) {
 ...
 * Strictly enforces Gmail settings: host='imap.gmail.com', port=993, tls/secure=true
 */
function getImapConfig(credentials = {}) {
  ...
  const isGmail = prov === 'gmail' || prov === 'google' || domain.includes('gmail') || domain.includes('googlemail') || (!imapHost && !prov && !domain);

  if (isGmail) {
    // Strictly verify Gmail required IMAP settings
...
/**
 * Attach detailed connection event listeners to IMAP client and socket
 * Catches connection drops, socket termination, and raw authentication rejection from server/Google
 */
function attachConnectionListeners(connection) {
  if (!connection) return;

  // Listeners on ImapSimple EventEmitter
  if (typeof connection.on === 'function') {
    connection.on('error', (err) => {
      console.error('🚨 [IMAP Client Error] Error event on IMAP client:');
      console.error(`   Message: ${err?.message || err}`);
      if (err?.textCode) console.error(`   Server Code: ${err.textCode}`);
      if (err?.source) console.error(`   Failure Phase: ${err.source}`);
      if (err?.code) console.error(`   System Error Code: ${err.code}`);
      if (err?.stack) console.error(`   Stack: ${err.stack}`);
    });

    connection.on('close', (hadError) => {
      if (hadError) {
        console.error('⚠️ [IMAP Client Close] Connection closed due to transmission/socket error.');
      } else {
        console.log('ℹ️ [IMAP Client Close] Connection closed normally.');
      }
    });

    connection.on('end', () => {
      console.log('ℹ️ [IMAP Client End] Connection stream ended.');
    });
  }

  // Raw listeners on underlying node-imap Connection socket (if present)
  if (connection.imap && typeof connection.imap.on === 'function') {
    connection.imap.on('error', (err) => {
      console.error('🚨 [IMAP Socket/Auth Error] Raw Google/Server rejection:');
      console.error(`   Raw details: ${err?.message || err}`);
      if (err?.textCode) console.error(`   IMAP Response Code: ${err.textCode}`);
      if (err?.source) console.error(`   Source: ${err.source}`);
    });

    connection.imap.on('close', (hadError) => {
      if (hadError) {
        console.error('⚠️ [IMAP Socket Close] Underlying socket closed with error.');
      }
    });

    connection.imap.on('alert', (alertMsg) => {
      console.warn('🔔 [IMAP Server Alert]:', alertMsg);
    });
  }
}

/**
 * Connect to IMAP server and strictly await authentication completion
 * Verifies settings and attaches detailed connection error listeners
 */
async function connectImap(credentials) {
  if (!credentials || !credentials.email || !credentials.password) {
    throw new Error('Mail client credentials (email and password/App Password) are required.');
  }

  const config = getImapConfig(credentials);
  console.log(`[IMAP] Connecting to ${config.imap.host}:${config.imap.port} for ${config.imap.user} (tls/secure: ${config.imap.tls})...`);

  let connection;
  try {
    // Strictly await connection and authentication
    connection = await imaps.connect(config);
    console.log('IMAP connected successfully.');
  } catch (err) {
    console.error('Failed to connect to IMAP:', err.message || err);
    if (err?.textCode) {
      console.error(`[IMAP Rejection Code]: ${err.textCode}`);
    }
    if (err?.source) {
      console.error(`[IMAP Failure Source]: ${err.source}`);
    }
    throw err;
  }
  ...
  // Attach error and close listeners to catch raw rejection & drops
...
async function testConnection(credentials) {
  ...
  try {
    ...
    // Await connection & authentication lifecycle
    connection = await connectImap(credentials);
...
async function fetchEmails(credentials, options = {}) {
  ...
  let connection;
  try {
    // Await full IMAP connection and authentication
    connection = await connectImap(credentials);
...
async function syncInbox(credentialsOrOptions = {}, maybeOptions = {}) {
  ...
  if (!credentials || !credentials.email || !credentials.password) {
    return {
      success: false,
      error: 'Mail client is not connected or initialized. Credentials are required.',
      emails: [],
      total: 0
    };
  }
  ...
  let connection;
  try {
    // Strictly await connection and authentication before opening mailbox or syncing
    connection = await connectImap(credentials);
    ...
    console.error('[syncInbox] Sync inbox failed:', err.message || err);
    if (connection?.end) {
      try { connection.end(); } catch (_) {}
    }
    return {
