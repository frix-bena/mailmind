...
import { extractDisplayName, isValidEmail } from './avatar-utils';
...
/**
 * Checks if an email belongs to an already existing/saved user on this device.
 */
export function isExistingUser(accountOrEmail) {
  if (!accountOrEmail || typeof window === 'undefined') return false;
  const email = typeof accountOrEmail === 'string' ? accountOrEmail : accountOrEmail?.email;
  if (!email || typeof email !== 'string') return false;
  const clean = email.toLowerCase().trim();
  if (isDemoAccount(clean)) return false;

  const accounts = getStoredAccounts();
  const existsInAccounts = accounts.some(a => a && a.email && a.email.toLowerCase().trim() === clean);
  if (existsInAccounts) return true;

  const active = getActiveUser();
  if (active && active.email && active.email.toLowerCase().trim() === clean) return true;

  return false;
}

/**
 * Update the password of an existing user account in localStorage and session.
 */
export function updateAccountPassword(email, newPassword) {
  if (!email || !newPassword || typeof window === 'undefined') return false;
  const clean = String(email).trim().toLowerCase();
  const accounts = getStoredAccounts();
  const index = accounts.findIndex(a => a && a.email && a.email.toLowerCase().trim() === clean);
  ...
  if (index !== -1) {
    ...
    const active = getActiveUser();
    if (active && active.email && active.email.toLowerCase().trim() === clean) {
      const updatedActive = {
        ...active,
        password: String(newPassword),
        savedAt: new Date().toISOString()
      ...
      try {
        localStorage.setItem(STORAGE_USER_KEY, JSON.stringify(updatedActive));
      } catch {}
    }
    return true;
  }
...
export function getActiveUser() {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.email && isValidEmail(parsed.email) && parsed.password && parsed.connected && !isDemoAccount(parsed)) {
        return parsed;
      }
      // If demo or invalid account found in localStorage, purge it
      if (parsed && (isDemoAccount(parsed) || !parsed.email || !isValidEmail(parsed.email) || !parsed.password || !parsed.connected)) {
...
export function addOrUpdateAccount(account) {
  if (!account || !account.email || !isValidEmail(account.email) || !account.password || isDemoAccount(account)) return null;
  const accounts = getStoredAccounts();
