...
/**
 * Calculate password strength score (1-4) and return visual feedback
 */
export function calculatePasswordStrength(password) {
  if (!password) return { score: 0, label: 'Empty', color: 'var(--muted)', width: '0%' };
  const clean = cleanAppPassword(password);
  let score = 0;

  if (clean.length >= 8) score += 1;
  if (clean.length >= 16) score += 1;
  ...
  switch (finalScore) {
    ...
    case 4:
    default:
      return { score: 4, label: 'Very Strong', color: '#22c55e', width: '100%' };
  }
}
