...
/**
 * Known brand and company domain mappings for rich organization names.
 */
...
export function extractDisplayName(name, email) {
  ...
  if (domain && BRAND_DOMAINS[domain] && (userPart === 'support' || userPart === 'notifications' || userPart === 'team' || userPart === 'no-reply' || userPart === 'hello')) {
    return `${BRAND_DOMAINS[domain]} ${userPart.charAt(0).toUpperCase() + userPart.slice(1)}`;
  }
 ...
 */
export function getGravatarUrl(email, size = 120) {
  const clean = extractCleanEmail(email);
 ...
 */
export function getClearbitLogoUrl(emailOrDomain, size = 120) {
 ...
 */
export function getUnavatarUrl(emailOrDomain, size = 120) {
 ...
 */
export function getGoogleProfileUrl(email, size = 120) {
  const clean = extractCleanEmail(email);
  if (!clean) return null;
 ...
 */
export function getAvatarSources(email, size = 120, allowClearbit = true) {
  const clean = extractCleanEmail(email);
  if (!clean) return [];

  const targetSize = Math.max(80, Math.round(size * 2));
...
/**
 * Assembles a comprehensive Sender Profile object for modals and rich previews.
 */
